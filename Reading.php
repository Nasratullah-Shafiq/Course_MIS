...
?>
    	...
    	<div class="container container-index">
    		<div class="row">
                ...
                <form method = "POST" action = "Reading-Ans?id=<?php echo $Book_ID; ?>" id = "QuizAnswer">
                <?php 
                    $i=1;
                    $a=0;
                    $con = mysqli_connect("localhost", "root", "", "Change");
                    $count = mysqli_query($con, "SELECT * from Reading where Book_ID = '$Book_ID' and Status = 1");  ?>
                    <table class="table">
                       <thead >
                        <tr>
                            <th> <h3> Test Your Reading Here </h3> </th>
                        </tr>
                        </thead> 
                    </table>
                    ...
                    <thead>
                    <tr>
                        <th> <h4 style="font-family: calibri;"> <?php echo $row['Body'];?> </h4> </th>
                    </tr>
                </thead>
                <thead>
                    <tr>
                ...
                <?php if(isset($row['Answer0'])){ ?>
                    <tr>
                        <td>&nbsp;1 &nbsp;<input type="radio" id = "<?php echo 'AnsZero'.$a;?>" value = "0"  name = "question[<?php echo $row['Reading_ID']; ?>]">&nbsp;<?php echo $row['Answer0'];?></td>
                    </tr>
                <?php }?>
                <?php if(isset($row['Answer1'])){ ?>
                    <tr>
                        <td>&nbsp;2 &nbsp;<input type="radio" id = "<?php echo 'AnsOne'.$a;?>" value = "1" name = "question[<?php echo $row['Reading_ID']; ?>]" >&nbsp;<?php echo $row['Answer1'];?></td> 
                    </tr>
                <?php }?>
                <?php if(isset($row['Answer2'])){ ?>
                    <tr>
                        <td>&nbsp;3 &nbsp;<input type="radio" id = "<?php echo 'AnsTwo'.$a;?>" value = "2" name = "question[<?php echo $row['Reading_ID']; ?>]">&nbsp;<?php echo $row['Answer2'];?></td>
                    </tr>
                <?php }?>
                <?php if(isset($row['Answer3'])){ ?>
                    <tr>
                        <td>&nbsp;4 &nbsp;<input type="radio" id = "<?php echo 'AnsThree'.$a;?>" value = "3" name = "question[<?php echo $row['Reading_ID']; ?>]">&nbsp;<?php echo $row['Answer3'];?></td>
                    </tr>
                <?php }?>
                    <tr style = "display: none;">
                        <td> <input type="radio" checked="checked" style = "display: none;" value = "No_Attempt" name = "question[<?php echo $row['Reading_ID']; ?>]"/></td>
                    </tr>
                </tbody> 
                </table>
                <?php $i++; $a++;}}
